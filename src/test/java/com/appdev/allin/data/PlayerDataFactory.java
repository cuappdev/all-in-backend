...
public class PlayerDataFactory {
  ...
  public static PlayerData createRandomPlayerData(Player player, OpposingTeam opposingTeam) {
    LocalDate gameDate =
        LocalDate.now()
            .minusDays(faker.number().numberBetween(1, 365)); // Random date within the past year
    ...
    Integer fieldGoals =
        faker.number().numberBetween(0, points / 2); // Field goals made (estimate based on points)
