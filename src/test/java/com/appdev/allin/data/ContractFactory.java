...
public class ContractFactory {
  ...
  public Contract createRandomContract(User owner) {
    ...
    Double buyPrice =
        faker.number().randomDouble(2, 100, 5000); // Random buy price between 100 and 5000
    ...
    Integer eventThreshold =
        faker.number().numberBetween(1, 100); // Random event threshold between 1 and 100
    LocalDate creationTime =
        LocalDate.now().minusDays(faker.number().numberBetween(1, 365)); // Within the past year
    ...
    Double sellPrice =
        forSale
            ...
            : null; // If for sale, random sell price
